import { LayoutDashboard, FileText, PlusCircle, LogOut, Search, Moon, Sun, X, Users, TrendingUp, Calendar, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export const Sidebar = ({ isOpen, onClose, isPinned, isCollapsed, onToggleCollapse }: SidebarProps) => {
    const { currentUser, logoutUser } = useAppStore();
    const { theme, toggleTheme } = useTheme();

    // When pinned the sidebar stays visible, so navigating must not close it.
    const handleNavClick = () => {
        if (!isPinned) onClose();
    };

    const isCompact = isPinned && isCollapsed;
    ...
    return (
        <>
            ...
            >
                <div className={clsx(
                    "flex h-24 items-center border-b border-white/20 dark:border-white/5 bg-transparent",
                    isCompact ? "justify-center px-4" : "justify-between px-8"
                )}>
                    ...
                    {!isCompact && (
                        isPinned ? (
                            <button
                                ...
                                title="Reducir menú"
                            >
                                <PanelLeftClose className="w-5 h-5 text-slate-500 hover:text-orange-500 transition-colors" />
                            </button>
                        ) : (
                ...
                <nav className={clsx("space-y-1 mt-2 flex-1 overflow-y-auto no-scrollbar", isCompact ? "p-3" : "p-4")}>
                        ...
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                clsx(
                                    ...
                                    isCompact ? 'justify-center py-3.5' : 'gap-3 px-5 py-3.5',
                                    {
                ...
                <div className={clsx("bg-transparent border-t border-white/20 dark:border-white/5 mt-auto", isCompact ? "p-3" : "p-4")}>
                    {isCompact && (
                        <button
                            onClick={onToggleCollapse}
                            className="flex w-full items-center justify-center h-10 mb-3 rounded-2xl bg-white/30 dark:bg-slate-950/20 ring-1 ring-white/20 dark:ring-white/5 text-slate-500 hover:text-orange-500 transition-colors"
                            title="Ampliar menú"
                        >
                    ...
                    {isCompact ? (
                        <button
                            onClick={toggleTheme}
                            className="flex w-full items-center justify-center h-10 mb-4 rounded-2xl bg-white/30 dark:bg-slate-950/20 ring-1 ring-white/20 dark:ring-white/5 transition-colors"
                    ...
                    <div
                        ...
                        title={isCompact ? (currentUser?.name || 'Usuario') : undefined}
                        className={clsx(
                            "group flex items-center rounded-2xl bg-white/40 dark:bg-slate-900/15 border border-white/20 dark:border-white/5 hover:border-orange-400/50 dark:hover:border-orange-500/30 transition-colors duration-200 cursor-pointer mb-3 shadow-sm",
